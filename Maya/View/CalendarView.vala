...
public class CalendarView : Gtk.HBox {
    ...
    void set_date (DateTime date) {
        ...
        debug (@"Setting focus date to $(selected_date)");
    ...
    void on_show_weeks_changed () {
        ...
        weeks.update (selected_date, show_weeks);
    }
    ...
    public void on_model_parameters_changed () {
        ...
        if (model.target.get_month() != selected_date.get_month() || model.target.get_year() != selected_date.get_year()) {
            int year_diff = selected_date.get_year() - model.target.get_year();
            int month_diff = selected_date.get_month() - model.target.get_month();
            var date = selected_date.add_full (year_diff, month_diff, 0, 0, 0, 0);
            set_date (date);
        }
    }
